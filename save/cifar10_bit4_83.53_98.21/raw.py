...
import math
import torch
from layer.quantize import QuantizeFc, QuantizeConv, QuantizeSignedActSBN
...
@jit(target="cpu", nopython=True)
def raw_quantize(x, bias, shift, n_bit):
    asrt_int(x)
    asrt_int(bias)
    asrt_int(shift)
...
def raw_inference(x, net):
    for layer in net.conv:
        if isinstance(layer, QuantizeConv):
            ...
            x = raw_quantize(x, layer.bias.numpy(), layer.shift.numpy(), layer.n_bits)
    ...
    with open(file_path, "w") as f:
        ...
        for layer in net.fc:
            if isinstance(layer, QuantizeFc):
                ...
                x = raw_quantize(x, bias, shift, layer.n_bits)
