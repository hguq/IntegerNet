...
def raw_inference(x, net):
    for layer in net.conv:
        if isinstance(layer, QuantizeConv):
            ...
            x = raw_quantize(x, layer.bias.numpy(), layer.shift.numpy(), layer.n_bits, signed=True)
        elif isinstance(layer, ReLUQuantizeUnsignedActSBN):
            x = raw_quantize(x, layer.bias.numpy(), layer.shift.numpy(), layer.n_bits, signed=False)
...
def save_raw_parameter(file_path):
    ...
    with open(file_path, "w") as f:
        for layer in net.conv:
            if isinstance(layer, QuantizeConv):
            ...
            elif isinstance(layer, (QuantizeSignedActSBN, ReLUQuantizeUnsignedActSBN)):
                signed = isinstance(layer, QuantizeSignedActSBN)
                bias = layer.bias.numpy()
                shift = -layer.shift.numpy()  # here shift is always negative, so change it to positive

                f.write("SIGNEDQUAN " if signed else "UNSIGNEDQUAN")
                _, C, H, W = x.shape
                ...
                x = raw_quantize(x, layer.bias.numpy(), layer.shift.numpy(), layer.n_bits, signed=signed)
        ...
        for layer in net.fc:
            if isinstance(layer, QuantizeFc):
                ...
                x = raw_quantize(x, bias, shift, layer.n_bits, signed=signed)
